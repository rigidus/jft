...
#+BEGIN_SRC elisp
  ...
  (int-to-binary-string 7)

  (defun get-cmd-plist-x86--mov-reg-to-reg (from to)
    (list :op #x8B
          :mod #b11 ;; direct-register
          :reg (get-x86-reg from)
          :rm  (get-x86-reg to)))

  (get-cmd-plist-x86--mov-reg-to-reg "%ESI" "%RDI")

  (defun cmd-plist-to-bytes (in)
    `(,(getf in :op)
      ,(car (read-from-string (concat "#b" (getf in :mod) (getf in :reg) (getf in :rm))))))

  (mapcar #'(lambda (in)
              (format "%X" in))
          (cmd-plist-to-bytes
           (get-cmd-plist-x86_64--mov-reg-to-reg "%EAX" "%EBX")))
#+END_SRC
