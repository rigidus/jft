...
*** START push_and_pop_reg_x86_64
[TODO:kvt] Тут можно сделать описание того как устроена команда...
...
#+BEGIN_SRC elisp
  (defun cmd-short-plist-to-bytes (in)
    (let ((out)
      ...
      (setf out (ash op 3))
      (setf out (logior out reg))
      out
      ))

    (defun get-cmd-plist-x86--push-reg (target)
  ...
  (mapcar #'(lambda (in)
          ...
          (cmd-short-plist-to-bytes
           (nth 0 (get-cmd-plist-x86--pop-reg :%esi)))
#+END_SRC
