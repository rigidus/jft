  ...
  (defun p-aasm (param)
    (block ram
      ...
      (let* ((ident     (- (length param) (length (string-trim-left param))))
        ...
        (replace-regexp-in-string "^#\\(.*\\)" ;; comment-line
                                  (lambda (in)
        ...
        (save-match-data ;; mov mem to reg
          (and (string-match (concat "^mov\s*(" reg ")\s*,\s*" reg "\\(.*\\)") param)
               (return-from ram `(mov_mem_to_reg :from  ,(match-string 1 param)
                                                 :to    ,(match-string 2 param)
                                                 :rest  ,(match-string 3 param)
                                                 :ident ,ident))))
...
- (mov_mem_to_reg :from "%IPR" :to "%ACR" :rest "   # mov mem to reg" :ident 4)
- (mov_mem_to_reg :from "%IPR" :to "%ACR" :rest "" :ident 4)
